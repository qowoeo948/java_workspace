...
class LoginForm extends Frame{
	...
	public LoginForm(){
		...
		//특히 부품은, 이객체가 태어날때 같이 함께 태어나야 하므로, 생성자의 타이밍을 놓치지말자
		la_id = new Label("ID");
		...
		//this.setLayout(new BorderLayout()); 필요없다. 왜 ? 디폴트가 이미 보더레이아웃이다
		//Color-> 처음보는 객체에 대한 대응법
		//1. 뭐하는 객체인지 파악 2. 메모리에 올리는법(일반,추상,인터페이스)
		p_center.setBackground(new Color(102,204,0));
		//센터패널을 윈도우의 BorderLayout 센터에 넣자!!
		this.add(p_center,BorderLayout.CENTER); //CENTER니까 생략가능		

		//상수는 직관성을 부여한 데이터이다. final로 더이상 값을 변경할 수 없으며, static으로 인스턴스간
		//공유가 가능하며  public으로 선언하여, 모든 객체가 접근할 수 있도록 접근 제한을 풀어놓은 데이터
