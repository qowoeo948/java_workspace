...
class Car{
	//아래의 변수는 클래스로 부터 인스턴스가 생성될때, 해당 인스턴스에 포함되어 지는
	//인스턴스 변수로써, 생명력은 해당 인스턴스가 소멸될때까지 함께 한다..

	//자동차의 부품이므로, 자동차와 생명을 같이 한다..
    //이러한 멤버변수는, 초기화 않으면 컴파일러가 최소한의 관여로 값을 자동초기화 시킨다.
	...
	//자동차의 인스턴스를 한개 생성한후, 그 인스턴스 안의 멤버변수들이 갖는
	//값들을 출력해보자!!(정말로 컴파일러에 의해 초기화 되었는지....)
