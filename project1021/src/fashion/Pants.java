...
2)관련성 있는 파일끼리 묶을 수 있음, 관리가 용이
	...
	.java와 같은 원본 소스를 만들때는, 개발자가 일일이 디렉토리를 직접 생성해야 하지만
	컴파일처럼 자동으로 기계어로 만들어 질때는, 이 디렉토리가 자동으로 만들어져야 편할것이다
*/
//의미는 아래와 같이 해야하지만 실질적으로는 아래와 같이 작성하면 안된다!!
//이유? 자바는 writer once run anywhere !!!
